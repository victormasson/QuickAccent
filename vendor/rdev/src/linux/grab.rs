...
{
    grab_with_is_repeat(move |event, _is_repeat| callback(event))
}

// QuickAccent patch: like `grab`, but tells the callback whether the key
// event is a kernel autorepeat (EV_KEY value 2). Stock rdev collapses
// repeats into KeyPress, making them indistinguishable from real presses.
pub fn grab_with_is_repeat<T>(callback: T) -> Result<(), GrabError>
where
    T: Fn(Event, bool) -> Option<Event> + 'static,
{
    ...
    filter_map_events(|event| {
        let is_repeat = matches!(&event.event_code, EventCode::EV_KEY(_)) && event.value == 2;
        ...
        if callback(rdev_event, is_repeat).is_some() {
...
{
    let (epoll_fd, mut devices, mut output_devices) = setup_devices()?;
        ...
        'events: for event in &epoll_buffer[0..num_events] {
            ...
            if event.data == INOTIFY_DATA {
                for event in inotify.read_events(&mut inotify_buffer)? {
                    ...
                    // QuickAccent patch: non-fatal + grabs hot-plugged devices
    ...
    T: AsRef<Path>,
{
    ...
    for entry in read_dir(path)? {
        ...
        match File::open(&path) {
            ...
            Err(e) => eprintln!("[rdev] skipping {}: {e}", path.display()),
        }
    }
    Ok(res)
}
...
// the ones present at startup, so a keyboard connected mid-session (USB
// replug, Bluetooth reconnect) is suppressible too. Virtual (uinput-created)
// devices are skipped: grabbing our own uinput clones would clone them again,
// forever. All failures are non-fatal — a single odd device node must not
// kill the whole grab loop.
fn add_device_to_epoll_from_inotify_event(
    epoll_fd: RawFd,
    event: inotify::Event<&OsStr>,
    devices: &mut Vec<Device>,
    output_devices: &mut Vec<UInputDevice>,
) {
    let Some(name) = event.name else { return };
    // Same filter as get_device_files: mouse*/mice/js* nodes don't play nice
    // with libevdev.
    let name_bytes = name.as_bytes();
    if name_bytes.starts_with(b"mouse") || name_bytes.starts_with(b"js") || name_bytes == b"mice" {
        return;
    }
    if is_uinput_device(name) {
        return;
    }
    let mut device_path = OsString::from(DEV_PATH);
    device_path.push(OsString::from("/"));
    device_path.push(name);
    // new plug events
    let Ok(file) = File::open(&device_path) else {
        return;
    };
    ...
    if device.grab(evdev_rs::GrabMode::Grab).is_err() {
        return;
    }
    // devices and output_devices must stay parallel (indexed by epoll data).
    let event = epoll::Event::new(EPOLLIN, devices.len() as u64);
    devices.push(device);
    output_devices.push(out_device);
    let _ = epoll::ctl(epoll_fd, EPOLL_CTL_ADD, fd, event);
}

/// Is /dev/input/<name> backed by a uinput-created device?
///
/// uinput devices have no parent, so they sit directly under
/// /sys/devices/virtual/input/. Bluetooth keyboards are also "virtual" but
/// live under /sys/devices/virtual/misc/uhid/.../input/ — those are real
/// keyboards and must NOT match.
fn is_uinput_device(name: &OsStr) -> bool {
    let sys = Path::new("/sys/class/input").join(name).join("device");
    std::fs::canonicalize(sys)
        .map(|p| p.starts_with("/sys/devices/virtual/input"))
        .unwrap_or(false)
}
